...
public class PlayerRisingHeatEffectRenderer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    public PlayerRisingHeatEffectRenderer(RenderLayerParent<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> pRenderer) {
        super(pRenderer);
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int pPackedLight, AbstractClientPlayer abstractClientPlayer, float pLimbSwing, float pLimbSwingAmount, float pPartialTicks, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if(ModCapabilities.getHeatedState(abstractClientPlayer).isPresent()) {
            ModCapabilities.getHeatedState(abstractClientPlayer).ifPresent(heated -> {
                if(heated.isHeated()) {
